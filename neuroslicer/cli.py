...
def _load_knowledge_base(args: argparse.Namespace) -> KnowledgeBase:
    if args.kb_markdown_dir:
        return KnowledgeBase.from_markdown_dir(args.kb_markdown_dir)

    if not args.no_guide_autodetect:
        guide_dir = detect_troubleshooting_guide()
        if guide_dir is not None:
            return KnowledgeBase.from_markdown_dir(guide_dir)

    return KnowledgeBase.from_json(args.kb_json)
